...
// shadow bans
export async function setShadowBan(userId, on, client = null, actorId = null) {
    const run = client ? client.query.bind(client) : query;

    const { rows } = await run(
        `WITH changed AS (
             UPDATE users SET shadow_banned = $2::boolean, updated_at = now()
              WHERE user_id = $1
          RETURNING user_id, name, shadow_banned
         ), audited AS (
             INSERT INTO activity_log (${LOG_COLUMNS})
             SELECT CASE WHEN $2::boolean THEN 'user.shadowbanned' ELSE 'user.unshadowbanned' END,
                    $3::int, user_id, name, NULL, NULL, NULL, NULL
               FROM changed
         )
         SELECT user_id, shadow_banned FROM changed`,
        [userId, Boolean(on), actorId]
    );
    return rows[0] ?? null;
}
...
export async function shadowBanProjectOwner(projectId, actorId) {
    const { rows } = await query(
        `WITH changed AS (
             UPDATE users u SET shadow_banned = true, updated_at = now()
               FROM projects p
