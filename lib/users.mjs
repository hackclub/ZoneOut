...
const USER_COLUMNS =
    ...
    "is_banned, ban_reason, shadow_banned, region, cg_access, hackatime_user_id, hackatime_linked_at, hackatime_synced_at, " +
...
export async function getUserWithProjects(userId) {
    const { rows } = await query(
        `SELECT u.user_id, u.email, u.slack_id, u.name, u.status,
                ...
                u.is_banned, u.ban_reason, u.shadow_banned, u.region, u.hackatime_user_id,
                ...
                COALESCE((
                    ...
                    WHERE p.user_id = u.user_id
                      AND (NOT u.shadow_banned OR p.project_id = u.shadow_project_id)
                ), '[]'::json) AS projects
         FROM users u
         WHERE u.user_id = $1`,
        [userId]
...
export async function getProjectWithViewer(projectId, viewerId) {
    const { rows } = await query(
        `SELECT ${PROJECT_COLUMNS},
                ...
                (SELECT u.shadow_banned FROM users u WHERE u.user_id = $2) AS viewer_shadow,
                (SELECT u.shadow_project_id FROM users u WHERE u.user_id = $2) AS viewer_shadow_project,
                (SELECT o.shadow_banned FROM users o WHERE o.user_id = projects.user_id) AS owner_shadow_banned,
        ...
        `SELECT ${PROJECT_COLUMNS}
         ...
         WHERE user_id = $1
           AND NOT EXISTS (SELECT 1 FROM users u
                            WHERE u.user_id = $1 AND u.shadow_banned
                              AND u.shadow_project_id IS DISTINCT FROM projects.project_id)
    ...
    const { rows } = await query(
        `WITH moved AS (
             ...
             WHERE project_id = $1 AND user_id = $2
               ...
               AND NOT EXISTS (SELECT 1 FROM users u WHERE u.user_id = $2 AND u.shadow_banned)
...
export async function queueProjectForReview(projectId, actorId, client = null) {
    ...
    const { rows } = await run(
        `WITH prior AS (
             ...
             FROM prior
             WHERE project_id = prior_id
             RETURNING ${PROJECT_COLUMNS}
         ), restored AS (
             UPDATE event_participants e SET
                 event_hours      = e.event_hours + p.prior_event,
                 deflation_adjust = e.deflation_adjust + p.prior_deducted
               FROM prior p JOIN queued q ON q.project_id = p.prior_id
              WHERE e.user_id = p.prior_user AND p.prior_fraud
                AND (p.prior_event > 0 OR p.prior_deducted > 0)
          RETURNING e.user_id,
                    GREATEST(e.event_hours + e.hours_adjust - e.deflation_adjust, 0)::float8 AS board_hours
         ), unmarked AS (
             INSERT INTO activity_log (${LOG_COLUMNS})
             SELECT 'event.restored', $2::int, q.user_id, q.owner_name, q.project_id, NULL,
                    (p.prior_event - p.prior_deducted)::numeric,
                    'fraud lifted · leaderboard now ' || r.board_hours
               FROM queued q
               JOIN prior p ON p.prior_id = q.project_id
               JOIN restored r ON r.user_id = q.user_id
         ), logged AS (
             INSERT INTO project_reviews (project_id, status, remarks, actor_id, hours)
    ...
    const { rows } = await run(
        `WITH prior AS (
             ...
             SELECT money.*,
                    ...
                    END AS board_hit,
                    CASE WHEN $6::boolean AND NOT prior_fraud THEN 1
                         WHEN NOT $6::boolean AND prior_fraud THEN -1
                         ELSE 0 END AS fraud_shift
             FROM money
         ), decided AS (
             UPDATE projects SET
             ...
             RETURNING ${PROJECT_COLUMNS},
                       ...
                       fraud_shift, prior_event AS shift_event, prior_deducted AS shift_deducted
         ), boarded AS (
             UPDATE event_participants e SET
                 event_hours      = GREATEST(e.event_hours - d.fraud_shift * d.shift_event, 0),
                 deflation_adjust = GREATEST(e.deflation_adjust + d.board_hit::numeric
                                             - d.fraud_shift * d.shift_deducted, 0)
               FROM decided d
              WHERE e.user_id = d.user_id
                AND (d.board_hit > 0
                     OR (d.fraud_shift <> 0 AND (d.shift_event > 0 OR d.shift_deducted > 0)))
          RETURNING e.user_id,
                    GREATEST(e.event_hours + e.hours_adjust - e.deflation_adjust, 0)::float8 AS board_hours
         ), ledger AS (
             ...
             INSERT INTO activity_log (${LOG_COLUMNS})
             SELECT CASE WHEN d.fraud_shift < 0 THEN 'event.restored' ELSE 'event.deflated' END,
                    $4::int, d.user_id, d.owner_name, d.project_id, NULL,
                    (0 - d.board_hit - d.fraud_shift * (d.shift_event - d.shift_deducted))::numeric,
                    CASE WHEN d.fraud_shift > 0 THEN 'fraud reject · '
                         WHEN d.fraud_shift < 0 THEN 'fraud lifted · '
                         ELSE '' END || 'leaderboard now ' || b.board_hours
...
export async function listProjectsForReview() {
    const { rows } = await query(
        `SELECT p.project_id, p.user_id, p.name, p.owner_name,
                ...
                u.shadow_banned AS owner_shadow_banned
         FROM projects p
...
export async function listAllUsersForAdmin() {
    const { rows } = await query(
        `SELECT u.user_id, u.name, u.email, u.slack_id, u.status,
                u.balance_hours::float8 AS balance_hours,
                u.is_banned, u.ban_reason, u.shadow_banned, u.region, u.cg_access, u.created_at,
...
// shadow bans
export async function setShadowBan(userId, on, client = null, actorId = null, projectId = null) {
    const run = client ? client.query.bind(client) : query;

    const { rows } = await run(
        `WITH changed AS (
             UPDATE users SET
                 shadow_banned     = $2::boolean,
                 shadow_project_id = CASE WHEN $2::boolean
                                          THEN (SELECT p.project_id FROM projects p
                                                 WHERE p.project_id = $4::int AND p.user_id = $1)
                                          ELSE NULL END,
                 updated_at        = now()
              WHERE user_id = $1
          RETURNING user_id, name, shadow_banned, shadow_project_id
         ), audited AS (
             INSERT INTO activity_log (${LOG_COLUMNS})
             SELECT CASE WHEN $2::boolean THEN 'user.shadowbanned' ELSE 'user.unshadowbanned' END,
                    $3::int, user_id, name, shadow_project_id, NULL, NULL, NULL
               FROM changed
         )
         SELECT user_id, shadow_banned, shadow_project_id FROM changed`,
        [userId, Boolean(on), actorId, projectId]
    );
    return rows[0] ?? null;
}

export async function shadowBanProjectOwner(projectId, actorId) {
    const { rows } = await query(
        `WITH changed AS (
             UPDATE users u SET shadow_banned = true, shadow_project_id = p.project_id, updated_at = now()
               FROM projects p
              WHERE p.project_id = $1 AND u.user_id = p.user_id AND u.user_id <> $2
          RETURNING u.user_id, u.name, p.project_id
         ), audited AS (
             INSERT INTO activity_log (${LOG_COLUMNS})
             SELECT 'user.shadowbanned', $2::int, user_id, name, project_id, NULL, NULL, NULL
               FROM changed
         )
...
// section for the admin BANS panel
export async function listBans() {
    const { rows } = await query(
        `SELECT u.user_id, u.name, u.email, u.slack_id,
                u.is_banned, u.ban_reason, u.shadow_banned, u.board_shadowed,
                u.shadow_project_id, sp.name AS shadow_project_name,
                banned.created_at AS banned_at, banned.actor_name AS banned_by,
                shadowed.created_at AS shadowed_at, shadowed.actor_name AS shadowed_by,
                hammered.created_at AS hammered_at, hammered.actor_name AS hammered_by
           FROM users u
           LEFT JOIN projects sp ON sp.project_id = u.shadow_project_id
           LEFT JOIN LATERAL (
               SELECT a.created_at, actor.name AS actor_name
                 FROM activity_log a LEFT JOIN users actor ON actor.user_id = a.actor_id
                WHERE a.subject_id = u.user_id AND a.kind = 'user.banned'
                ORDER BY a.event_id DESC LIMIT 1) banned ON u.is_banned
           LEFT JOIN LATERAL (
               SELECT a.created_at, actor.name AS actor_name
                 FROM activity_log a LEFT JOIN users actor ON actor.user_id = a.actor_id
                WHERE a.subject_id = u.user_id AND a.kind = 'user.shadowbanned'
                ORDER BY a.event_id DESC LIMIT 1) shadowed ON u.shadow_banned
           LEFT JOIN LATERAL (
               SELECT a.created_at, actor.name AS actor_name
                 FROM activity_log a LEFT JOIN users actor ON actor.user_id = a.actor_id
                WHERE a.subject_id = u.user_id AND a.kind = 'event.shadowed'
                ORDER BY a.event_id DESC LIMIT 1) hammered ON u.board_shadowed
          WHERE u.is_banned OR u.shadow_banned OR u.board_shadowed
          ORDER BY u.user_id`
    );
    return rows;
}
