...
// profile, reduced to the fields we store
function normaliseIdentity(body) {
    const identity = body?.identity ?? body ?? {};

    const first = trimmed(identity.first_name ?? identity.given_name);
    const last = trimmed(identity.last_name ?? identity.family_name);

    const composed = [first, last].filter(Boolean).join(" ");
    const named = trimmed(identity.name);
