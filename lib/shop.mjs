...
import { MAIL_COLUMNS, MAIL_AUTHOR, orderMail } from "./mailbox.mjs";
...
const ORDER_SQL = `WITH charged AS (
          ...
          RETURNING user_id, name, region, balance_hours
             ...
             RETURNING order_id, user_id, user_name, item_name, quantity, hours_spent
         ), audited AS (
             INSERT INTO activity_log (${LOG_COLUMNS})
             SELECT 'order.placed', user_id, user_id, user_name, NULL, order_id, hours_spent,
    ...
    const { rows } = await query(
        `WITH removed AS (
          ...
          RETURNING balance_hours::float8 AS balance_hours
             ...
             INSERT INTO activity_log (${LOG_COLUMNS})
             SELECT 'order.cancelled', $1::int, $1::int,
                    (SELECT name FROM users WHERE user_id = $1),
                    NULL, order_id, hours_spent, item_name || ' x' || quantity
               FROM removed
         )
    ...
    const { rows } = await run(
        `WITH eligible AS (
          ...
          RETURNING ${ORDER_COLUMNS}
             ...
             SELECT replace($5::text, '{name}', d.item_name),
                    replace($6::text, '{name}', d.item_name)
                    || CASE WHEN EXISTS (SELECT 1 FROM credited)
                            THEN chr(10) || chr(10) || 'The ' || d.hours_spent
                                 || ' hours it cost are back on your balance.'
                            ELSE '' END,
                    $3::int, $7::text, d.user_id
               FROM decided d
         ), audited AS (
             INSERT INTO activity_log (${LOG_COLUMNS})
             SELECT CASE WHEN $2::text = 'approved' THEN 'order.approved'
                         ELSE 'order.rejected' END,
                    $3::int, user_id, user_name, NULL, order_id, hours_spent,
                    item_name || ' x' || quantity
                    || CASE WHEN EXISTS (SELECT 1 FROM credited) THEN ' (refunded)' ELSE '' END
               FROM decided
         )
         SELECT d.*, EXISTS (SELECT 1 FROM credited) AS refund_paid FROM decided d`,
        [orderId, status, actorId, refund === true, mail.title, mail.body, MAIL_AUTHOR]
    );
    return rows[0] ?? null;
}
...
export async function getBalanceHours(userId) {
    const { rows } = await query(
        `SELECT email, balance_hours::float8 AS balance_hours, is_banned, ban_reason,
                region, cg_access, shadow_banned
         FROM users WHERE user_id = $1`,
        [userId]
    );
    return rows[0] ?? null;
