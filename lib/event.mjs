...
export async function eventTotals() {
    const { rows } = await query(
        `SELECT COALESCE(SUM(CASE WHEN u.board_shadowed THEN 0
                                  ELSE ${EFFECTIVE_HOURS} END), 0)::float8 AS hours,
                COUNT(*)::int                                AS participants
         FROM event_participants
         JOIN users u ON u.user_id = event_participants.user_id
         WHERE hidden = false`
    );
...
// section for the leaderboard shadow ban
export async function setBoardShadow(userId, on, actorId = null) {
    const { rows } = await query(
        `WITH changed AS (
             UPDATE users SET board_shadowed = $2::boolean, updated_at = now()
              WHERE user_id = $1
          RETURNING user_id, name
         ), audited AS (
             INSERT INTO activity_log (${LOG_COLUMNS})
             SELECT CASE WHEN $2::boolean THEN 'event.shadowed' ELSE 'event.unshadowed' END,
    ...
    if (!rows[0]) throw new ValidationError(`No user with id ${userId}.`);
    return rows[0];
}
...
// every deflation and restore the board has taken, newest first
export async function boardDeflations() {
    const { rows } = await query(
        `SELECT a.event_id, a.kind, a.project_id, a.hours::float8 AS hours, a.detail, a.created_at,
                a.subject_id AS user_id, a.subject_name AS owner_name,
                u.slack_id,
                p.name AS project_name,
                COALESCE(p.fraud_rejected, false) AS fraud_rejected,
                (p.project_id IS NOT NULL) AS project_exists,
                actor.name AS actor_name
           FROM activity_log a
           LEFT JOIN users u     ON u.user_id = a.subject_id
           LEFT JOIN projects p  ON p.project_id = a.project_id
           LEFT JOIN users actor ON actor.user_id = a.actor_id
          WHERE a.kind IN ('event.deflated', 'event.restored')
...
// every account beside what it has logged toward the board
export async function boardUsers() {
    const { rows } = await query(
        `SELECT u.user_id,
                u.name,
                u.slack_id,
                u.email,
                (p.user_id IS NOT NULL)                  AS on_board,
                COALESCE(p.hidden, false)                AS hidden,
                u.board_shadowed                         AS shadowed,
                COALESCE(p.event_hours, 0)::float8       AS tracked,
                COALESCE(p.hours_adjust, 0)::float8      AS adjust,
                COALESCE(p.deflation_adjust, 0)::float8  AS deflation,
                ...
                (SELECT count(*) FROM projects pr
                  WHERE pr.user_id = u.user_id AND pr.event_hours > 0)::int AS projects,
                (SELECT COALESCE(SUM(GREATEST(pr.event_hours - pr.event_deducted, 0)), 0)
                   FROM projects pr WHERE pr.user_id = u.user_id
                    AND NOT pr.fraud_rejected)::float8 AS project_hours
         FROM users u
         LEFT JOIN event_participants p ON p.user_id = u.user_id
         ORDER BY on_board DESC, hours DESC NULLS LAST, u.user_id
         LIMIT 1000`
    );
    return rows;
}
        ...
        `WITH incoming AS (
          ...
          RETURNING p.project_id
             ...
             UPDATE event_participants
                SET event_hours = LEAST($2::numeric, COALESCE((
                        SELECT SUM(i.hours) FROM incoming i
                          JOIN projects p ON p.project_id = i.project_id
                         WHERE p.user_id = $1 AND NOT p.fraud_rejected), 0)),
                    synced_at   = now()
              WHERE user_id = $1
          RETURNING user_id
         )
