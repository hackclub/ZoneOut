...
export async function syncAllLinkedUsers({
    ...
    const userIds = await listLinkedUserIds();
    const deadline = Date.now() + budgetMs;
    const result = { linked: userIds.length, users: 0, projects: 0, failed: 0, skipped: 0 };

    let next = 0;

    async function worker() {
        while (next < userIds.length && Date.now() < deadline) {
    ...
    const workers = Math.min(concurrency, userIds.length);
