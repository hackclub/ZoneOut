...
import { sealToken, openToken } from "./secretbox.mjs";
import { query } from "./db.mjs";
import { ValidationError, normaliseHackatimeProject, findHackatimeLinkOwner, listHackatimeLinks } from "./users.mjs";
import { HACKATIME_SINCE, EVENT_SINCE } from "../catalog.js";
import { writeEventHours } from "./event.mjs";
...
// project hours since a given day
export async function fetchProjectStats(accessToken, since = HACKATIME_SINCE) {
    const params = new URLSearchParams({
        features: "projects",
        limit: "100",
...
export async function readToken(userId) {
    ...
    return openToken(rows[0].hackatime_token);
}

// one hackatime account belongs to one zoneout account
export class HackatimeTaken extends Error {
    constructor() {
        super("that hackatime account is already linked to another zoneout account");
        this.name = "HackatimeTaken";
    }
}
    ...
    const { rows } = await query(
        `UPDATE users SET
         ...
         WHERE user_id = $1
           ...
           AND NOT EXISTS (
               SELECT 1 FROM users held
               WHERE held.hackatime_user_id = $2 AND held.user_id <> $1
           )
         RETURNING user_id, hackatime_user_id`,
        [userId, hackatimeUserId, sealToken(accessToken)]
    );

    if (rows[0]) return rows[0];

    // the write matched nothing; one read says whether somebody else holds it
    const { rows: held } = await query(
        "SELECT user_id FROM users WHERE hackatime_user_id = $1 AND user_id <> $2",
        [hackatimeUserId, userId]
    );
    if (held[0]) throw new HackatimeTaken();
...
// event hours, counting only the projects linked to a zoneout project
export async function eventHoursFor(userId, accessToken) {
    const [stats, links] = await Promise.all([
        fetchProjectStats(accessToken, EVENT_SINCE),
        listHackatimeLinks(userId)
    ]);

    const owner = new Map();
    for (const link of links) {
        const name = (link.hackatime_project || "").trim().toLowerCase();
        if (name) owner.set(name, link.project_id);
    }
    ...
    for (const project of stats) {
        const projectId = owner.get(project.name.trim().toLowerCase());
        if (projectId === undefined) continue;
        projects.push({ projectId, hours: project.hours });
        total += project.hours;
    }

    return { total: Math.round(total * 100) / 100, projects };
}
...
// the administrator's sweep of every linked account
const SWEEP_CONCURRENCY = 4;
const SWEEP_BUDGET_MS = 10_000;

async function listLinkedUserIds() {
    const { rows } = await query(
        `SELECT u.user_id, (e.user_id IS NOT NULL) AS in_event
         FROM users u
         LEFT JOIN event_participants e ON e.user_id = u.user_id
         WHERE u.hackatime_user_id IS NOT NULL
           AND u.hackatime_token IS NOT NULL
           AND EXISTS (
               SELECT 1 FROM projects p
               WHERE p.user_id = u.user_id AND p.hackatime_project IS NOT NULL
           )
         ORDER BY u.hackatime_synced_at NULLS FIRST, u.user_id`
    );
    return rows.map(row => ({ userId: row.user_id, inEvent: row.in_event }));
}
...
export async function syncAllLinkedUsers({
    ...
    const targets = await listLinkedUserIds();
    const deadline = Date.now() + budgetMs;
    const result = { linked: targets.length, users: 0, projects: 0, event: 0, failed: 0, skipped: 0 };

    let next = 0;

    async function worker() {
        while (next < targets.length && Date.now() < deadline) {
            const { userId, inEvent } = targets[next++];
            try {
                ...
                // both windows in parallel, so the sweep is a universal event recount too
                const [stats, event] = await Promise.all([
                    fetchProjectStats(token),
                    eventHoursFor(userId, token)
                ]);

                const settled = await writeProjectHours(userId, stats);
                await writeEventHours(userId, event.total, event.projects);
                if (inEvent) result.event++;

                result.users++;
                result.projects += settled.length;
            } catch (err) {
                result.failed++;
                console.error(`hackatime sweep failed for user ${userId}:`, err.message);
            }
        }
    }
    ...
    result.skipped = result.linked - result.users - result.failed;
    return result;
}
