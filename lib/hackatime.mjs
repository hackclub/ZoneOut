...
export async function eventHoursFor(userId, accessToken) {
    ...
    const wanted = new Set(
        links
            .map(link => (link.hackatime_project || "").trim().toLowerCase())
            .filter(Boolean)
    );

    let total = 0;
    for (const project of stats) {
        if (wanted.has(project.name.trim().toLowerCase())) total += project.hours;
    }
        ...
        while (next < targets.length && Date.now() < deadline) {
            ...
            try {
                ...
                const [stats, eventHours] = await Promise.all([
                    ...
                    inEvent ? eventHoursFor(userId, token) : Promise.resolve(null)
                ]);

                const settled = await writeProjectHours(userId, stats);
                if (eventHours !== null) {
                    await writeEventHours(userId, eventHours);
                    result.event++;
                }
