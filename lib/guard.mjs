...
// a shadow-banned user may look but not change anything
export function refuseReadOnly(user, res) {
    if (!user?.shadow_banned) return false;
    res.status(409).json({ ok: false, error: "not allowed" });
    return true;
}
...
export async function resolveAdmin(req) {
    ...
    if (!user || user.is_banned || user.shadow_banned) return null;
